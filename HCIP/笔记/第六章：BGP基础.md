...
mindmap
  root((BGP 边界网关协议))
    (("一、产生背景与核心概念"))
      ("协议定义：AS间的动态路由协议，属于EGP")
      ("自治系统(AS)：统一管理，运行同一IGP协议")
      ("AS编号：分为2字节和4字节，含公有和私有")
    (("二、路由协议分类"))
      ("IGP：RIP、OSPF、IS-IS")
      ("EGP：BGP")
    (("三、BGP协议特性"))
      ("传输机制：AS间传路由，内部扩散靠IGP")
      ("路径矢量协议：一跳为一个AS")
      ("防环机制：AS_PATH属性防环")
      ("传输层：基于TCP协议，端口179，需手动建邻")
      ("更新机制：首次发完整路由表，后续增量更新")
      ("控制：多种属性控制，支持聚合、过滤、策略")
    (("四、BGP基本术语"))
      ("BGP Speaker：运行BGP协议的路由器")
      (("BGP Peer (对等体)"))
        ("EBGP对等体：跨AS邻居，一般物理直连")
        ("IBGP对等体：同一个AS内部的邻居")
    (("五、BGP规划问题(路由黑洞)"))
      ("产生原因：AS内有未运行BGP的路由器")
      (("解决方法"))
        ...
        ("配置静态路由：不现实")
        ("IBGP全连接：工作量大")
        ("BGP路由反射器：无视IBGP防环限制减少邻居")
        ("BGP联盟：避开IBGP水平分割限制")
    (("六、BGP环路问题(水平分割)"))
      ("EBGP防环：AS_PATH含本地AS号则拒绝学习")
      ("IBGP防环：IBGP学到的路由不通告给其他IBGP")
    (("七、BGP消息种类"))
      ("头部：包含Marker、Length、Type")
      ("Open：建连接，携带router-id与保持时间")
      ("Keepalive：周期保活，默认60s")
      ("Update：路由更新（增减），含属性与NLRI")
      ("Notification：检测到错误，关闭连接")
      ("Router-refresh：策略改变后要求重发路由表")
      ("ORF：出口路由过滤器，按需接收")
    (("八、BGP状态机"))
      ("Idle：空闲，准备TCP连接")
      ("Connect：TCP被动连接中")
      ("Active：TCP主动连接中")
      ("Open-sent：发出Open，等对方Open")
      ("Open-confirm：收到Open，发出Keepalive")
      ("Established：收到Keepalive，邻居建立")
    (("九、BGP邻居建立条件"))
      (("IBGP"))
        ("物理口：建议使用直连IP")
        ("环回口：TCP可达，需修改更新源地址")
      (("EBGP"))
        ("物理口：建议使用直连IP")
        ("环回口：TCP可达，需修改更新源地址")
        ("非直连必做：修改EBGP最大跳数(ebgp-max-hop)")
    (("十、BGP基本配置"))
      ("启动：bgp [AS号]，配router-id与peer")
      ("优化：connect-interface与ebgp-max-hop")
      ("路由发布：network发布，import-route引入")
      ("查看状态：display bgp peer / routing-table")
```
