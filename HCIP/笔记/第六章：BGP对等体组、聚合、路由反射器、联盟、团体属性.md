...
mindmap
  root((大规模BGP网络问题与解决))
    ...
    (("二、对应的解决方案概览"))
      ("对等体众多：对等体组、BGP团体属性")
      ("路由表庞大：BGP路由聚合")
      ("IBGP全连接：路由反射器(RR)、BGP联盟")
      ("路由频繁变化：BGP路由衰减")
    (("三、对等体组 (Peer Group)"))
      ("定义：具有相同属性的对等体集合，简化配置")
      (("分类"))
        ("IBGP对等体组")
        ("EBGP对等体组：需同一AS，跨AS配置麻烦")
    (("四、BGP路由聚合"))
      ("作用：减小路由表规模")
      (("自动聚合"))
        ("仅对引入的IGP生效，对自身宣告无效")
        ("汇总到主类，易致路由黑洞，华为默认关闭")
        ("S(suppressed)状态代表路由被抑制")
      (("手工聚合 (aggregate)"))
        ("精确汇总，任意路由器均可配置")
        ("detail-suppressed：抑制明细路由，防环路")
        ("as-set：继承AS-PATH属性防环")
        ("suppress-policy：通告汇总并选择性抑制明细")
    (("五、BGP路由反射器 (RR)"))
      ("作用：替代IBGP全连接，减少邻居数，解路由黑洞")
      (("角色划分"))
        ("RR (路由反射器)")
        ("Client (客户机)：无需做任何配置")
        ("非客户机")
      (("反射规则"))
        ...
        ("从EBGP接收：反射给所有")
      (("防环机制 (解决水平分割失效的环路)"))
        ("Cluster_list (集群列表)：防反射器间环路，记录经过的cluster_id")
        ("Originator_ID (起源ID)：防跨反射群环路，记录始发者Router ID")
    (("六、BGP联盟 (Confederation)"))
      ("原理：将大AS划分为若干子AS，内部全连接，子AS间建立联盟EBGP")
      (("注意点"))
        ("子AS使用私有AS编号")
        ("跨子AS的EBGP邻居需更改下一跳为本机")
        ("实际中改变了邻居关系，较少使用")
    (("七、BGP团体属性 (Community)"))
      ("定义：路由标记，批量匹配特征路由，便于策略控制")
      (("公认属性"))
        ("Internet：可通告给所有对等体")
        ("no-advertise：不发送给任何对等体")
        ("no-export：不发给EBGP，可发给联邦内")
        ("no-export-subconfed：不发给任何EBGP(含联邦)")
      ("注意：需开启advertise-community传递社团属性")
    (("八、BGP路由衰减 (Route Dampening)"))
      ("作用：解决路由振荡 (Route flaps) 问题")
      (("参数与机制"))
        ("惩罚值：每震荡一次固定加1000")
        ("抑制阈值 (默认2000)：超此值路由被抑制")
        ("重用阈值 (默认750)：衰减至此值下重新通告")
        ("半衰期 (默认15分钟)：惩罚值随时间减半")
        ("最大抑制时间 (默认60分钟)：抑制时间上限")
```
